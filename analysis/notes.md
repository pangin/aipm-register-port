...
| Tag | ToolTip (제품명, 한국어) | 보조 prefix (m_c) | 선택 SSID 기반 모델 코드 |
|---|---|---|---|
| `S120`   | 스마트 플러그 (16A)        | `DWD-LS120` | 주 `B530_W`, 보조 `B540_W` |
| `ES120`  | 스마트 플러그 (16A)        | `DWD-SS120` | 주 `B550E_W`, 보조 `B550_W` |
| `LS130`  | 스마트 플러그 (16A)        | (없음)      | `B350_W` |
| `S220`   | 스마트 멀티탭 (16A)        | (없음)      | `M130_W` |
| `LS810`  | 지그비 허브                | (없음)      | `G200L_ZB` |
| `S510`   | IR 리모컨                  | (없음)      | `R110_W` |
| `S501`   | IR 리모컨                  | `DWD-S510`  | 주/보조 `R200_W` |
| `S310`   | 분전반 단상 (50A)          | `DWD-S311`  | 주 `P110_W`, 보조 `P110_WA` |
| `S330`   | 분전반 3상 (100A)          | (없음)      | `P230_W` |
| `S350`   | 분전반 3상 (400A)          | (없음)      | `P250_W` |
| `S370`   | 분전반 3상 (800A)          | (없음)      | `P270_W` |
| `ES120S` | 태양광 스마트 플러그 (16A) | `DWD-ES120` | 주/보조 `B550E_SW` |
| `S600`   | 태양광 스마트 플러그 (10A) | (없음)      | `B400_SW` |
| `S110`   | 스마트 플러그 (10A)        | `DWD-S600`  | 주 `B400_WI`, 보조 `B400_W` |
| `S121`   | 일본향 스마트 플러그       | (없음)      | `B343_W` |

**Hotspot SSID 필터**: 4/5 화면은 선택된 제품의 `m_B = "DWD-" + Tag` (주 prefix) + `m_c` (보조 prefix) 둘 중 하나로 시작하는 SSID 만 표시.

**모델 코드 추출**: 선택된 장치 hotspot SSID(`this.m_b`)의 첫 토큰을 `Split('_')[0].Substring(4)` 로 잘라 (예: `DWD-S120` → `S120`) `_3()` switch 로 매핑. 결과가 `device_id = "DAWONDNS-{model}-{mac}"` 의 model 부분이며, TCP 응답 문자열은 모델 코드로 쓰이지 않는다.
